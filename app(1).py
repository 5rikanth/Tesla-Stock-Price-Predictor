...
Upload your TSLA CSV file and select a prediction horizon to get started.
""")
...
if uploaded_file is None:
    st.info("👈 Please upload your TSLA.csv file from the sidebar to get started.")
    st.stop()
...
ax.plot(df.index, df["MA_30"], linewidth=1.5, color="#FF9800", linestyle="--", label="30-Day MA")
ax.set_ylabel("Price ($)")
...
for i, (name, preds) in enumerate(all_preds.items()):
    with cols[i]:
        direction = "🟢" if preds[-1] >= latest_close else "🔴"
        st.metric(
            f"{name} — Day 1",
            f"${preds[0]:.2f}",
...
if model_choice == "All Three":
    ...
    for name, preds in all_preds.items():
        summary_rows.append({
            ...
            "Predicted Change ($)": round(preds[-1] - latest_close, 2),
            "Predicted Change (%)": round(
                (preds[-1] - latest_close) / latest_close * 100, 2
            ),
        })
    ...
    # RMSE comparison
    st.subheader("📊 RMSE Performance Comparison")

    rmse_df = pd.DataFrame({
        "Forecast Horizon": ["1 Day", "5 Day", "10 Day"],
        "SimpleRNN RMSE": [23.82, 47.25, 70.11],
        "GRU RMSE": [20.73, 33.09, 43.54],
        "LSTM RMSE": [16.02, 28.57, 40.15]
    })
...
LSTM achieved the lowest RMSE across all prediction horizons.

• 1-Day RMSE: 16.02
• 5-Day RMSE: 28.57
• 10-Day RMSE: 40.15

Therefore, LSTM was selected as the final deployed forecasting model.
""")
...
with st.expander("📚 About the Models"):
    st.markdown("""
**SimpleRNN** — Basic recurrent unit. Updates a single hidden state each step.
Struggles with long sequences due to the vanishing gradient problem.

**GRU (Gated Recurrent Unit)** — Uses a reset gate and update gate to control
how much past information to retain. Lighter than LSTM, often comparable in accuracy.

**LSTM (Long Short-Term Memory)** — Three gates (input, forget, output) plus a
dedicated cell state. Best at capturing long-range dependencies in stock price history.
GridSearchCV was used to tune its hyperparameters (units, dropout, learning rate).

**Input Window:** Last 60 trading days of closing prices (normalized 0–1).  
